package InformationSystemLab.lab3;

import java.io.File;
import java.io.FileNotFoundException;
import java.util.Arrays;
import java.util.Collections;
import java.util.List;
import java.util.Scanner;

public class DES {

    private static final int[] T_IP = {
            58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4,
            62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8,
            57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3,
            61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7
    };

    private static final int[] T_FP = {
            40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31,
            38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29,
            36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27,
            34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25
    };

    private static final int[] T_E = {
            32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9,
            8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17,
            16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25,
            24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1
    };

    private static final int[] T_P = {
            16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10,
            2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25
    };

    private static final int[] T_PC1 = {
            57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18,
            10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36,
            63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22,
            14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4
    };

    private static final int[] T_PC2 = {
            14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4,
            26, 8, 16, 7, 27, 20, 13, 2, 41, 52, 31, 37, 47, 55,
            30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53,
            46, 42, 50, 36, 29, 32
    };

    private static final int[] SHIFTS = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};

    private static final int[][][] SBOX = {
            {{14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7},
                    {0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8},
                    {4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0},
                    {15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13}},
            {{15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10},
                    {3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5},
                    {0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15},
                    {13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9}},
            {{10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8},
                    {13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1},
                    {13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7},
                    {1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12}},
            {{7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15},
                    {13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9},
                    {10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4},
                    {3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14}},
            {{2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9},
                    {14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6},
                    {4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14},
                    {11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3}},
            {{12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11},
                    {10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8},
                    {9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6},
                    {4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13}},
            {{4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1},
                    {13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6},
                    {1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2},
                    {6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12}},
            {{13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7},
                    {1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2},
                    {7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8},
                    {2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11}}
    };

    public static int[] fromString(String s) {
        int[] b = new int[s.length()];
        for (int i = 0; i < s.length(); i++) {
            b[i] = s.charAt(i) - '0';
        }
        return b;
    }

    public static String toStringBits(int[] b) {
        StringBuilder sb = new StringBuilder(b.length);
        for (int bit : b) {
            sb.append(bit);
        }
        return sb.toString();
    }

    public static int[] applyPerm(int[] in, int[] table, int n) {
        int[] out = new int[n];
        for (int i = 0; i < n; i++) {
            out[i] = in[table[i] - 1];
        }
        return out;
    }

    public static int[] shiftLeft(int[] half, int n) {
        int[] out = new int[28];
        for (int i = 0; i < 28; i++) {
            out[i] = half[(i + n) % 28];
        }
        return out;
    }

    public static int[] xorBits(int[] a, int[] b) {
        int[] out = new int[a.length];
        for (int i = 0; i < a.length; i++) {
            out[i] = a[i] ^ b[i];
        }
        return out;
    }

    public static int[][] keySchedule(int[] key) {
        int[] perm = applyPerm(key, T_PC1, 56);
        int[] C = Arrays.copyOfRange(perm, 0, 28);
        int[] D = Arrays.copyOfRange(perm, 28, 56);

        int[][] subkeys = new int[16][];
        for (int round = 0; round < 16; round++) {
            C = shiftLeft(C, SHIFTS[round]);
            D = shiftLeft(D, SHIFTS[round]);

            int[] cd = new int[56];
            System.arraycopy(C, 0, cd, 0, 28);
            System.arraycopy(D, 0, cd, 28, 28);

            subkeys[round] = applyPerm(cd, T_PC2, 48);
        }
        return subkeys;
    }

    public static int[] roundFn(int[] R, int[] subkey) {
        int[] expanded = applyPerm(R, T_E, 48);
        int[] mixed = xorBits(expanded, subkey);

        int[] sout = new int[32];
        int idx = 0;
        for (int box = 0; box < 8; box++) {
            int base = box * 6;
            int row = (mixed[base] << 1) | mixed[base + 5];
            int col = (mixed[base + 1] << 3) | (mixed[base + 2] << 2)
                    | (mixed[base + 3] << 1) | mixed[base + 4];
            int val = SBOX[box][row][col];
            for (int b = 3; b >= 0; b--) {
                sout[idx++] = (val >> b) & 1;
            }
        }
        return applyPerm(sout, T_P, 32);
    }

    public static int[] desCore(int[] block, int[][] subkeys) {
        int[] data = applyPerm(block, T_IP, 64);
        int[] L = Arrays.copyOfRange(data, 0, 32);
        int[] R = Arrays.copyOfRange(data, 32, 64);

        for (int round = 0; round < 16; round++) {
            int[] newR = xorBits(L, roundFn(R, subkeys[round]));
            L = R;
            R = newR;
        }

        int[] combined = new int[64];
        System.arraycopy(R, 0, combined, 0, 32);
        System.arraycopy(L, 0, combined, 32, 32);
        return applyPerm(combined, T_FP, 64);
    }

    public static int[] encrypt(int[] plaintext, int[] key) {
        return desCore(plaintext, keySchedule(key));
    }

    public static int[] decrypt(int[] ciphertext, int[] key) {
        int[][] sk = keySchedule(key);
        List<int[]> skList = Arrays.asList(sk);
        Collections.reverse(skList);
        return desCore(ciphertext, skList.toArray(new int[0][]));
    }

    public static boolean isValid64BitBinary(String s) {
        return s != null && s.matches("[01]{64}");
    }

    public static void main(String[] args) {
        String filePath = "InformationSystemLab/lab3/input.txt";

        try (Scanner fileScanner = new Scanner(new File(filePath))) {
            if (!fileScanner.hasNextLine()) {
                System.err.println("[Error] File is empty.");
                return;
            }
            String ptStr = fileScanner.nextLine().trim();

            if (!fileScanner.hasNextLine()) {
                System.err.println("[Error] Key is missing.");
                return;
            }
            String keyStr = fileScanner.nextLine().trim();

            if (!isValid64BitBinary(ptStr) || !isValid64BitBinary(keyStr)) {
                System.err.println("Error: Both plaintext and key must be exactly 64 binary bits.");
                return;
            }

            int[] plaintext = fromString(ptStr);
            int[] key = fromString(keyStr);

            int[] ciphertext = encrypt(plaintext, key);
            int[] recovered = decrypt(ciphertext, key);

            System.out.println("Plaintext: " + ptStr);
            System.out.println("Key: " + keyStr);
            System.out.println("Ciphertext: " + toStringBits(ciphertext));
            System.out.println("Decrypted: " + toStringBits(recovered));

        } catch (FileNotFoundException e) {
            System.err.println("Error: File not found at '" + filePath + "'. Please check the path.");
        }
    }
}