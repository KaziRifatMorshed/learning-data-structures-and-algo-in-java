...
team 1. But team 1 and team 0 are not weaker than any other teams. So the answer is 
-1.
...
class findChampionII {
    public int findChampion(int n, int[][] edges) {
        ...
        Arrays.fill(parentCount, 0);

        // node gulor parent count korbo
        for (int i = 0; i < m; i++) {
            parentCount[edges[i][1]]++;
        }

        // jodi ekta node er parent count zero hoy then oita champion
        // otherwise -1
        int initCount = 0, champion = 0;
        for (int i = 0; i < n; i++) {
            if (parentCount[i] == 0) {
                initCount++;
                champion = i;
