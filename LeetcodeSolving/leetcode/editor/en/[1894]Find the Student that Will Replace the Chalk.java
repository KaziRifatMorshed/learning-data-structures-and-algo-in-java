...
//number 1, and so on until the teacher reaches the student number n - 1. After that,
...
//
//
// Related Topics Array Binary Search Simulation Prefix Sum 👍 1181 👎 132


//leetcode submit region begin(Prohibit modification and deletion)
class Solution {
    public int chalkReplacer(int[] chalk, int k) {
        int len = chalk.length, i = 0;
        if (k == 0) return 0;
        while (true) {
            k -= chalk[i];
            if (k <= 0) return i;
