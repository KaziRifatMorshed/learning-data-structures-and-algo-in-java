...
class constructMaximumBinaryTree {
    private TreeNode constructMBT(int[] arr, TreeNode node, int leftIdx, int rightIdx) {
        if (leftIdx <= rightIdx) {
            ...
            node.left = constructMBT(arr, node.left, leftIdx, maxIdx - 1);
            node.right = constructMBT(arr, node.right, maxIdx + 1, rightIdx);
            return node;
        } else return null;
    } // DONE

    public TreeNode constructMaximumBinaryTree(int[] nums) {
        return constructMBT(nums, null, 0, nums.length - 1);
    } // DONE
}
