...
class Kruskal2 {
    ...
    static int MAX_CAPACITY = Integer.MAX_VALUE;
    ...
    static boolean hasCycle(ArrayList<Integer>[] graph, int currentNode) {
        // visit korbo
        visited.set(currentNode, true);
        // adjacent traverse korbo
        for (int adjacentNode : graph[currentNode]) {
            if (hasCycle(graph, adjacentNode))
                if (visited.get(adjacentNode) == true) {
                    return false;
                }
        }
        // checking
